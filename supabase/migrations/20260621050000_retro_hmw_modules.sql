-- New runnable workshop module types from the design — Retrospective (a Feedback
-- board with Start/Stop/Continue lanes) and How-might-we (a Brainstorm framed as
-- an opportunity prompt). Both reuse the proven IdeaModule renderer; only the
-- enum + the live-path allow-lists need to know about them. (CREATE OR REPLACE
-- preserves existing grants.)

alter type public.activity_type add value if not exists 'retrospective';
alter type public.activity_type add value if not exists 'hmw';
