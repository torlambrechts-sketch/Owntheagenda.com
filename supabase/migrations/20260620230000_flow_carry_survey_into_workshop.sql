...
-- A Flow stamps workshop.survey_id (a link) but the run never read it and the
-- Play templates had no survey step, so the team ran a generic workshop with no
-- sight of what they'd just answered.
--
...
--
-- NOTE: program_autobuild here is rebased on the per-step-template version
-- (20260620210000) so this migration adds the attach call WITHOUT reverting the
-- per-step template selection. It is the last definition of program_autobuild.
...
-- Auto-build (per-step template, rebased on 20260620210000) + attach the survey.
