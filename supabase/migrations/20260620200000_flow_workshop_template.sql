-- =====================================================================
-- Flow workshop template — Phase K. Let a composed Flow pick WHICH workshop
-- to run up front (by template), stored as auto_workshop_template so the
-- workshop auto-builds when the response gate trips — the same mechanism
-- Plays already use. Flows that leave it unset keep the manual "Build
-- workshop" picker on the active workshop step.
-- =====================================================================

drop function if exists public.create_flow_steps(uuid, text, uuid, int, jsonb, text, int);
...
declare v_id uuid; v_n int := greatest(3, coalesce(p_min_responses, 3)); v_ord int := 0; r record;
        v_cd int := greatest(1, coalesce(p_collect_days, 7)); v_tmpl uuid;
begin
  ...
  -- Validate the chosen workshop template belongs to this workspace (or is global).
  ...
  for r in select * from jsonb_array_elements(p_steps) with ordinality as e(elem, ord) loop
    ...
    values (
      ...
      case (r.elem ->> 'kind')
        ...
        when 'workshop' then case when v_tmpl is not null then 'Auto-builds when the threshold is met'
...
grant execute on function public.create_flow_steps(uuid, text, uuid, int, jsonb, text, int, uuid) to authenticated;
revoke execute on function public.create_flow_steps(uuid, text, uuid, int, jsonb, text, int, uuid) from public, anon;
