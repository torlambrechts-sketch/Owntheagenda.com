...
export default async function TemplateBuilderPage({
  ...
  // Plus the curated OwnTheAgenda library — proven, sourced items grouped by
  // topic — so the bank is useful even before a workspace has authored its own
  // instruments. Deduped against instrument-derived items by text.
  for (const bi of ITEM_BANK) {
    const text = bi.text.trim();
    if (!text || seenText.has(text.toLowerCase())) continue;
    seenText.add(text.toLowerCase());
    bankItems.push({ text, instrument: bi.topic, source: bi.source, dimension: bi.dimension });
  }
