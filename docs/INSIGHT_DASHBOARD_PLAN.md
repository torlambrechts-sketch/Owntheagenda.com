...
- **Phase D — Reports subsystem ✅** Reports tab now live: scheduled-reports table (format/cadence/recipients/next-run/status + per-row ⋯ Send now / Pause-Resume / Delete), New-report side-window (name · recipient chips · format pdf/excel/csv · cadence once/weekly/monthly · includes · message), Quick-export card (CSV real, Excel best-effort .xls, PDF via browser print), Import-responses side-window (CSV parse + preview → anonymized merge), and a Recent-activity run log. Backend: server actions `listReports`/`createReport`/`setReportStatus`/`deleteReport`/`sendReportNow`/`importResponses` on `report_schedule`/`report_run`; migration `20260626120000_report_dispatch` adds the `report_cron` secret, `private.dispatch_reports()` (pg_net → edge function), admin-gated `request_report_dispatch()` + `import_survey_responses()`, and a daily `report-dispatch` pg_cron job; edge function **send-reports** (Resend, verify_jwt=false, x-cron-secret auth) drains queued/due work, generates HTML+CSV, advances recurring schedules. Sends stay **inert** until `RESEND_API_KEY` + a verified sender are set as function secrets — runs then log as `failed` with the reason. Verified: tsc+build green; RLS sim (admin create/read OK · import RPC inserted 3 · non-admin insert blocked · rolled back); dispatch smoke `200 {ok:true,processed:0}`; browser walkthrough created a one-off → run logged `failed: Email delivery not configured` + schedule auto-paused (sandbox data cleaned).

- **Phase E — Nav ✅** Shell Insight group → **Insights** (the `/insight` dashboard, rail target). Trends + Reports nav items removed; `/insight/trends` and `/insight/reports` are redirects → `/insight`.

- **Post-E — Leadership Teams removed ✅** (supersedes the earlier "management preserved" decision, per product owner). Deleted `app/(app)/insight/leadership-teams/` (page + HealthClient + health-actions, incl. the manual-RAG override surface) and its nav item. Repointed all internal links to `/insight`: dashboard "Team health" card + bars, workflow step links (launch/interpret/repulse), `/health` redirect, and `setSurveyShare`'s `revalidatePath`. Verified: grep shows zero `insight/leadership-teams` code refs; tsc + build green (route gone).
