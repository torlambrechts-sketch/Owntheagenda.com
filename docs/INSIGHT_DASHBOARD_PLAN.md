...
- **Phase E — Nav + preserve management ✅** Shell Insight group → **Insights** (the `/insight` dashboard, rail target) + **Leadership Teams** (kept); Trends + Reports nav items removed; `navItemActive("/insight")` matches exactly `/insight` so the sub-page lights its own item. `/insight/trends` and `/insight/reports` are now redirects → `/insight`. (Old leadership-teams management surface untouched.)
